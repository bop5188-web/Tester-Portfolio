(function () {
  ...
  /* --- Home: step snap-assist (one scroll step = next stop) --- */
  if (isHomePage) {
    ...
    var snapPendingIndex = -1;
    var snapUnlockTimer = null;
    var snapLastStepAt = 0;
    var snapWheelAccum = 0;
    var snapWheelDirection = 0;
    var snapAssistMotionReduce =
      window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    var wheelStepEnabled = window.matchMedia("(pointer: fine)").matches;

    function homeSnapTopOffset() {
      return topChrome && topChrome.classList.contains("is-visible") ? 66 : 0;
    }

    function nearestSnapIndex() {
      if (!snapStops.length) return -1;
      var topOffset = homeSnapTopOffset();
      var best = -1;
      var bestDist = Infinity;
    ...
    function goToSnapIndex(index) {
      if (!snapStops.length) return;
      var idx = Math.max(0, Math.min(index, snapStops.length - 1));
      snapAssistLock = true;
      snapPendingIndex = idx;
      snapLastStepAt = Date.now();
      if (snapUnlockTimer) {
        window.clearTimeout(snapUnlockTimer);
      }
      snapStops[idx].scrollIntoView({
        behavior: snapAssistMotionReduce ? "auto" : "smooth",
        block: "start",
      });
      snapUnlockTimer = window.setTimeout(function () {
        snapAssistLock = false;
        snapPendingIndex = -1;
      }, snapAssistMotionReduce ? 180 : 920);
    }

    function stepSnap(deltaY) {
      if (!snapStops.length) return false;
      var direction = deltaY > 0 ? 1 : -1;
      var projectsTop = projectsTitleStop ? projectsTitleStop.offsetTop : 0;

      /* From hero, first scroll down lands on Explore Projects first */
      if (direction > 0 && projectsTitleStop && window.scrollY < projectsTop - 24) {
        goToSnapIndex(0);
        return true;
      }

      var active = nearestSnapIndex();
      if (active < 0) active = 0;
      var next = active + direction;

      /* Let native scroll continue beyond bounds (footer / very top) */
      if (next < 0 || next >= snapStops.length) return false;
    ...
    if (wheelStepEnabled) {
      window.addEventListener(
        "scroll",
        function () {
          if (!snapAssistLock || snapPendingIndex < 0) return;
          var topOffset = homeSnapTopOffset();
          var dist = Math.abs(snapStops[snapPendingIndex].getBoundingClientRect().top - topOffset);
          if (dist < 20 && Date.now() - snapLastStepAt > 220) {
            snapAssistLock = false;
            snapPendingIndex = -1;
            if (snapUnlockTimer) {
              window.clearTimeout(snapUnlockTimer);
      ...
      window.addEventListener(
        ...
        function (e) {
          ...
          if (direction !== snapWheelDirection) {
            snapWheelDirection = direction;
            snapWheelAccum = 0;
          }
          snapWheelAccum += Math.abs(e.deltaY);
          /* Require deliberate wheel intent; ignores trackpad momentum micro-events */
          if (snapWheelAccum < 26) return;
          snapWheelAccum = 0;
          if (Date.now() - snapLastStepAt < 420) return;
          if (!stepSnap(e.deltaY)) return;
          e.preventDefault();
        },
        { passive: false }
      );
