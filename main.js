(function () {
  ...
  /* --- Home: project cards fall up into place while scrolling --- */
  if (isHomePage && caseStudySnaps.length) {
    ...
    function updateCaseFallUp() {
      fallRaf = 0;
      if (fallReduce) {
        for (var r = 0; r < caseStudySnaps.length; r++) {
          var settled = caseStudySnaps[r];
          var settledSlide = settled.querySelector(".case-study-slide");
          settled.classList.add("is-settled");
          if (!settledSlide) continue;
          settledSlide.style.setProperty("--fall-y", "0px");
          settledSlide.style.setProperty("--fall-o", "1");
          settledSlide.style.setProperty("--fall-s", "1");
          settledSlide.style.setProperty("--fall-rot", "0deg");
        }
        return;
      }
      ...
      for (var i = 0; i < caseStudySnaps.length; i++) {
        ...
        /* 0 = still below viewport, 1 = landed near center */
        var start = vh * 0.92;
        var end = vh * 0.28;
        var raw = (start - rect.top) / (start - end);
        var p = easeOutCubic(clamp01(raw));
        var y = (1 - p) * 34;
        var o = 0.16 + p * 0.84;
        var s = 0.93 + p * 0.07;
        var rot = (1 - p) * (i % 2 === 0 ? 2.6 : -2.2);
        slide.style.setProperty("--fall-y", y.toFixed(2) + "vh");
        slide.style.setProperty("--fall-o", o.toFixed(3));
        slide.style.setProperty("--fall-s", s.toFixed(3));
        slide.style.setProperty("--fall-rot", rot.toFixed(2) + "deg");
        snap.classList.toggle("is-settled", p > 0.97);
      }
    }

    function scheduleCaseFallUp() {
      if (fallRaf) return;
      fallRaf = window.requestAnimationFrame(updateCaseFallUp);
    }

    updateCaseFallUp();
    window.addEventListener("scroll", scheduleCaseFallUp, { passive: true });
    window.addEventListener("resize", scheduleCaseFallUp, { passive: true });
    window.addEventListener("load", scheduleCaseFallUp);
  }
